...
vec3 SampleColoredCube(vec3 vv) {
	vec3 v = abs(vv);
	if (v.x >= v.y && v.x >= v.z) {
		if (vv.x > 0) return vec3(1, 0, 0);
		return vec3(1, 1, 0);
	}
	if (v.y >= v.x && v.y >= v.z) {
		if (vv.y > 0) return vec3(0, 1, 0);
		return vec3(0, 1, 1);
	}
	if (v.z >= v.y && v.z >= v.x) {
		if (vv.z > 0) return vec3(0, 0, 1);
		return vec3(1, 0, 1);
	}
	return vec3(0);
}
...
vec3 prefilterEnvMap(vec3 R, float roughness)
{
	...
	float envMapDim = float(textureSize(samplerEnv, 0).s) / 3.f;
